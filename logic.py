...
from model import User2, UserCollection, Student, Tutor, Competency, Qualification, Contract, Subject, OpenBid, CloseBid, \
    MessageClass

from data import DBActions
...
class UserAdapter:
    ...
    def create_user(self, user_dict): 
        ...
        dtd = DBActions().create_user(user_dict)
        print('dtd----', dtd)
        # user2 = User2(user_dict)
    ...
    def get_all_users(self):
        ...
        for u in all_users_dict:
            ...
            #     if 'monitoredBids' in u['additional_info']:
            #         monitor_bids_ids = u['additional_info']['monitoredBids']
    ...
    def monitor_bid(self, tutor, bid):
        ...
        if successful:
            ...
            update_api_body = {"additional_info":
                {
...
class ContractAdapter:
    ...
    def get_all_contracts(self):
        ...
        for con in all_contracts_dict:
            ...
            # Creating contract
            contract = Contract(con['id'], first_party, second_party, subject, con['date_created'], con['dateSigned'],
                                con['expiryDate'])
            contract.set_additional_info(con['additional_info'])
    ...
    def add_contract_api(self, contract, sign):
        ...
        add_contract = {
            ...
            "date_created": contract.get_date_created(),
            "expiryDate": contract.get_expiry_date(),
            "paymentInfo": {},
            "lessonInfo": contract.get_lesson_info(),
            "additional_info": contract.get_additional_info()
        }

        Contract_API_access().add_new_contract(add_contract)
        if sign:
            id = self.get_contract_id(contract)
            date_signed_calculate = dateutil.parser.parse(add_contract["date_created"][:-1])  + timedelta(
                minutes=1)
...
class BidAdapter:
    ...
    def get_all_bids(self):
        ...
        print('self.bids----', bids_API)
        for b in bids_API:

            if b['date_closed'] is None:
                subject = SubjectAdapter().create_subject(b["subject"])
                u = UserAdapter().create_contract_student(b['initiator'])
                if b['type'] == 'open':
                    bid = OpenBid(b['id'], u, b['date_created'], b['date_closed'], subject)
                    bid.set_additional_info(b['additional_info'])

                    if dateutil.parser.parse(b['date_created'][:-1]) + timedelta(
                            minutes=30) <= datetime.now():
                        CloseDownBid(u, bid).close_down_by_time()
                    else:
                        all_bids.append(bid)
                else:
                    bid = CloseBid(b['id'], u, b['date_created'], b['date_closed'], subject)
                    bid.set_additional_info(b['additional_info'])
    ...
    def add_bid_api(self, initiator_id, subject_id, bid):
        ...
        bid_dictionary = {
            ...
            "date_created": bid.get_date_created(),
            "subjectId": subject_id,
            "additional_info": bid.get_additional_info()
        }
        # add new bid to API
...
class BidUpdater:
    ...
    def notify_subscribers(self):
        ...
        for s in self.subscribers:
            ...
            s.set_additional_info(bid_from_api['additional_info'])
    ...
    def get_messages(self, bid, offer):
        ...
        for i in range(len(all_messages_json)):
            all_messages.append(MessageClass(all_messages_json[i]['id'], all_messages_json[i]['bidId'],
                                             ...
                                             all_messages_json[i]['content'], all_messages_json[i]['additional_info']))
